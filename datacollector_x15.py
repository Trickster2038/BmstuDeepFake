...
# 15 - default, f1 = 0.60
HIST_SIZE_PERCENT = 10 # relative to face width
...
SAVEABLE = True
SAVE_PATH = '../deep_fake_src/dataset_frames/'
SAVE_FAIL_PATH = '../deep_fake_src/dataset_frames_fail/'
FOLDER_PATH = '../deep_fake_src/dfdc_train_part_27/'
METADATA_PATH = '../deep_fake_src/dfdc_train_part_27/metadata.json'
TARGET_FRAME_N = 10
...
if __name__ == "__main__":
    ...
    if LIMIT > 0 or OFFSET > 0:
         df_fakes = df_fakes.sort_values(by='filename').iloc[OFFSET:OFFSET+LIMIT]
    ...
    for index, row in df_fakes.iterrows():
        ...
        while vd.isOpened() and (cnt_inner <= TARGET_FRAME_N * TARGET_FRAME_AMOUNT):

            ret, frame = vd.read()
            cnt_inner += 1
            
            if vd.isOpened() and (not (frame is None)):
                if (cnt_inner > 0) and (cnt_inner % TARGET_FRAME_N == 0):
                    ...
                    if SAVEABLE and not(d is None):
                        cv.imwrite(SAVE_PATH + str(cnt_total) + '_' + filename + '_' + str(cnt_inner) + '.png', frame_result)
                    if SAVEABLE and (d is None):
                        cv.imwrite(SAVE_FAIL_PATH + str(cnt_total) + '_' + filename + '_' + str(cnt_inner) + '.png', frame_result)
